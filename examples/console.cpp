...
namespace console {
...
#if defined(_WIN32)
    static void*     hConsole         = nullptr;
#else
    static FILE*     tty              = nullptr;
    static termios   initial_state;
#endif

    // Cooperative end-of-stream flag. Set instead of calling
    // GenerateConsoleCtrlEvent() which would kill the whole process
    // (including audio/TTS worker threads). The main loop checks
    // is_eof() and shuts down gracefully.
    static bool      g_console_eof    = false;

#if defined(_WIN32)
    // Convert UTF-8 to UTF-16 (Windows only)
    ...
    // Convert UTF-16 to UTF-8 (Windows only)
    ...
    void init(bool use_simple_io, bool use_advanced_display) {
        ...
        // Enable ANSI/VT processing only if we actually own a valid console.
        if (hConsole != nullptr) {
            if (advanced_display &&
        ...
        if (hConIn != INVALID_HANDLE_VALUE && GetConsoleMode(hConIn, &dwMode)) {
            // Set console input codepage to UTF-16
            _setmode(_fileno(stdin), _O_WTEXT);

            // Enable/disable line input and echo depending on the IO mode
            if (simple_io) {
    ...
    void cleanup() {
        ...
        if (!simple_io) {
            ...
            // Always restore stdout, even if tty was never opened.
            out = stdout;
    ...
    static char32_t getchar32() {
        ...
        while (true) {
            INPUT_RECORD record;
            DWORD count = 0;
            if (!ReadConsoleInputW(hConsoleIn, &record, 1, &count) || count == 0) {
                g_console_eof = true;
                return static_cast<char32_t>(WEOF);
            }
            ...
            if (record.EventType == KEY_EVENT && record.Event.KeyEvent.bKeyDown) {
                ...
                if ((wc >= 0xDC00) && (wc <= 0xDFFF)) {
                    if (high_surrogate != 0) {
                        return static_cast<char32_t>(((high_surrogate - 0xD800) << 10) +
                                                     (wc - 0xDC00) + 0x10000);
                    }
                }
        ...
        if ((wc >= 0xD800) && (wc <= 0xDBFF)) {
            ...
            if ((low_surrogate >= 0xDC00) && (low_surrogate <= 0xDFFF)) {
                return static_cast<char32_t>((static_cast<char32_t>(wc & 0x03FF) << 10) +
    ...
    static void pop_cursor() {
        ...
        if (hConsole != nullptr) {
            ...
            GetConsoleScreenBufferInfo((HANDLE)hConsole, &bufferInfo);
            ...
            SetConsoleCursorPosition((HANDLE)hConsole, newCursorPosition);
            return;
        }
#endif
        putc('\b', out);
    ...
    static int put_codepoint(const char* utf8_codepoint, size_t length, int expectedWidth) {
#if defined(_WIN32)
        CONSOLE_SCREEN_BUFFER_INFO bufferInfo;
        if (hConsole == nullptr || !GetConsoleScreenBufferInfo((HANDLE)hConsole, &bufferInfo)) {
            // Fall back to expected width
            return expectedWidth;
        }
        COORD initialPosition = bufferInfo.dwCursorPosition;
        DWORD nNumberOfChars = (DWORD)length;
        WriteConsole((HANDLE)hConsole, utf8_codepoint, nNumberOfChars, &nNumberOfChars, NULL);

        CONSOLE_SCREEN_BUFFER_INFO newBufferInfo;
        GetConsoleScreenBufferInfo((HANDLE)hConsole, &newBufferInfo);

        // Figure out our real position if we're in the last column
        if (utf8_codepoint[0] != 0x09 && initialPosition.X == newBufferInfo.dwSize.X - 1) {
            DWORD nChars;
            WriteConsole((HANDLE)hConsole, " \b", 2, &nChars, NULL);
            GetConsoleScreenBufferInfo((HANDLE)hConsole, &newBufferInfo);
        }
        ...
        int results = fscanf(tty, "\033[%d;%dR", &y1, &x1);
    ...
    static void append_utf8(char32_t ch, std::string & out) {
        ...
        // else: invalid codepoint, ignore
    }

    // Note: the old pop_back_utf8_char() helper was removed.
    // It had a subtle bug: a 4-byte UTF-8 sequence needed up to
    // 3 continuation bytes to be scanned, but the loop stopped at
    // i < 3, leaving the first byte of a 4-byte sequence in place.
    // It was also unused — the advanced editor uses
    // prev_utf8_char_pos()/next_utf8_char_pos() instead.
    ...
    static char32_t decode_utf8(const std::string & input, size_t pos, size_t & advance) {
        ...
        if ((c & 0xE0u) == 0xC0u && pos + 1 < input.size()) {
            ...
            advance = 2;
            return static_cast<char32_t>(((c & 0x1Fu) << 6) | (c1 & 0x3Fu));
        }
        if ((c & 0xF0u) == 0xE0u && pos + 2 < input.size()) {
            ...
            advance = 3;
            return static_cast<char32_t>(((c & 0x0Fu) << 12) |
                                         ((c1 & 0x3Fu) << 6) |
                                         (c2 & 0x3Fu));
        }
        if ((c & 0xF8u) == 0xF0u && pos + 3 < input.size()) {
            ...
            advance = 4;
            return static_cast<char32_t>(((c & 0x07u) << 18) |
                                         ((c1 & 0x3Fu) << 12) |
                                         ((c2 & 0x3Fu) << 6) |
                                         (c3 & 0x3Fu));
        }
    ...
    static void move_cursor(int delta) {
        ...
        if (hConsole != nullptr) {
            ...
            while (newX >= width) { newX -= width; newY++; }
            while (newX < 0)      { newX += width; newY--; }

            newCursorPosition.X = (SHORT)newX;
            newCursorPosition.Y = (SHORT)newY;
            SetConsoleCursorPosition((HANDLE)hConsole, newCursorPosition);
        }
#else
        if (delta < 0) {
    ...
    static bool readline_advanced(std::string & line, bool multiline_input) {
        ...
        while (true) {
            ...
            if (input_char == static_cast<char32_t>(WEOF) || input_char == 0x04) {
                end_of_stream = true;
                break;
            }
            ...
            if (input_char == '\033') {
                // Skip ANSI escape sequences (arrow keys are handled above).
                ...
                if (code == '[') {
                    while (true) {
                        ...
                        if ((code >= 'A' && code <= 'Z') ||
                            (code >= 'a' && code <= 'z') ||
                            code == '~' ||
                            code == static_cast<char32_t>(WEOF)) {
                            break;
                        }
                    }
                }
    ...
    static bool readline_simple(std::string & line, bool multiline_input) {
        ...
        if (!std::getline(std::wcin, wline)) {
            // stdin is closed or redirected. Signal EOF cooperatively —
            // do NOT call GenerateConsoleCtrlEvent(), which would terminate
            // the whole process, including background threads.
            line.clear();
            g_console_eof = true;
            return false;
        }
        ...
        int size_needed = WideCharToMultiByte(CP_UTF8, 0,
                                              wline.data(), (int)wline.size(),
                                              NULL, 0, NULL, NULL);
        if (size_needed <= 0) {
            // Conversion failed; treat as empty line.
            line.clear();
            line += '\n';
            return multiline_input;
        }
        ...
        WideCharToMultiByte(CP_UTF8, 0,
                            wline.data(), (int)wline.size(),
                            &line[0], size_needed, NULL, NULL);
#else
        if (!std::getline(std::cin, line)) {
            // EOF on POSIX (Ctrl+D). Signal cooperatively.
            line.clear();
            g_console_eof = true;
            return false;
        }
