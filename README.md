...
- [2026.07.04] Stable: [v1.9.2](https://github.com/ggml-org/whisper.cpp/releases/tag/v1.9.2) / [Roadmap](https://github.com/orgs/ggml-org/projects/4/)
- [2026.07.04] Рафакторинг проекта. 
## Заметки
   -  В talk-llama.cpp был изменена работа с контекстом и кэшем под whisper.cpp > 1.8.0., и изменена работа с кэшем. В версии 1.76 это реализовывалось по другому.
   ...
   -  talk-llama.cpp тестировался на llm модели saiga_yandexgpt_8b.Q4_0.gguf и Whisper модели whisper-ggml-large-v3-q4.bin
   ...
   -  Далее, и сейчас все тесты проводятся на RTX 3060 12GB на архитектуре Ampere, так что проект придется перекомпилировать.
   ...
   -  Использовать бота желательно с наушниками: чтобы исключить эффект обратного эха в микрофон.  
...
- Windows 10/11 x64
- python, cuda 11, 12
- Recommended: nvidia GPU with 8-12 GB vram. Minimum: nvidia with 6 GB. 
...
Установка ffmpeg 
не понадобится, так как у нас tall-llama без wavtolip, для экономии ресурсов, но инструкция будет ниже:
...
ПРИМЕЧАНИЕ: При первом запуске XTTS скачает DeepSpeed с GitHub (27 МБ) и чекпоинта XTTS (1,8 ГБ). 
